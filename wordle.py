...
def makeAGuess(_guess):
  ...
  for i in range(len(word)):
    ...
    # TASK F: If the previous condition is fails, check if the current letter of guess is in word at all. If so add the letter "Y" to the hint
    if (_guess[i] in word):
      hint+= "Y";
      continue;
